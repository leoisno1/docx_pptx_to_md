...
from custom_types import MultiColumnSlide
...
def is_two_column_text(slide):
    if slide.slide_layout.name != "TITLE":
        all_mu = list()
        all_sigma = list()
        for shape in sorted(ungroup_shapes(slide.shapes), key=lambda x: (x.top, x.left)):
...
def assign_shapes(slide, params, ncols=2, slide_width_mm=1000):
    ...
    shapes = sorted(ungroup_shapes(slide.shapes), key=lambda x: (x.top, x.left))
    ...
    for shape in slide.shapes:
        ...
        if shape.shape_type == MSO_SHAPE_TYPE.PICTURE or shape.has_text_frame:
            ...
            curr_sigma = Length(shape.width / 4).mm
